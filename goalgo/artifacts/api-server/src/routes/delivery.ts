...
import { getShopUser } from "../lib/shop-auth.js";
...
/** Global alışveriş ürün kategori ağacı — modül kaldırıldı */
router.get("/ecommerce-product-categories", async (_req, res): Promise<void> => {
  res.status(410).json({ success: false, error: "Alışveriş modülü kaldırıldı", tree: [] });
});

router.get("/delivery/marketplace", async (_req, res): Promise<void> => {
  res.status(410).json({ success: false, error: "Alışveriş modülü kaldırıldı" });
});

/** Pazaryeri sepet özeti — modül kaldırıldı */
router.post("/delivery/marketplace/cart/preview", async (_req, res): Promise<void> => {
  res.status(410).json({ success: false, error: "Alışveriş modülü kaldırıldı" });
});

/** Çok satıcılı pazaryeri checkout — modül kaldırıldı */
router.post("/delivery/marketplace/checkout", async (_req, res): Promise<void> => {
  res.status(410).json({ success: false, error: "Alışveriş modülü kaldırıldı" });
});

router.get("/delivery/marketplace/products/:slugOrId", async (_req, res): Promise<void> => {
  res.status(410).json({ success: false, error: "Alışveriş modülü kaldırıldı" });
});

router.get("/delivery/marketplace/about", async (_req, res): Promise<void> => {
  res.status(410).json({ success: false, error: "Alışveriş modülü kaldırıldı" });
});
