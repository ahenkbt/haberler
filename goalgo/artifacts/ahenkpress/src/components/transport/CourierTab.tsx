...
const COURIER_TYPES = [
  { id: "document", label: "Belge / Evrak", icon: "📄" },
  { id: "package", label: "Küçük Kargo", icon: "📦" },
  { id: "food", label: "Yemek", icon: "🍱" },
  { id: "flower", label: "Çiçek", icon: "💐" },
  { id: "medicine", label: "İlaç", icon: "💊" },
...
export default function CourierTab() {
  ...
  async function submit() {
    if (!form.customerName || !form.customerPhone || !fromAddress || !toAddress) {
      alert("Ad, telefon, alış ve teslimat adresi zorunlu");
      return;
    }
    setLoading(true);
    const payload: Record<string, unknown> = {
      requestType: "courier",
      extraData: { itemType: selectedType },
  ...
  if (done)
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <h3 className="text-xl font-bold text-gray-900 mb-2">Kurye Talebi Alındı!</h3>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <h2 className="font-bold text-lg mb-1 text-gray-900">📦 Kurye Gönder</h2>
      <p className="text-sm text-gray-500 mb-5">Kapıdan kapıya hızlı teslimat</p>

      <div className="mb-4">
        <label className="text-xs text-gray-500 mb-2 block">Ne Gönderiyorsunuz?</label>
          ...
          {COURIER_TYPES.map((t) => (
            <button
              ...
              className={`flex flex-col items-center gap-1 p-2 rounded-xl border text-xs font-medium transition-all ${
                selectedType === t.id ? "border-orange-400 bg-orange-50 text-orange-700" : "border-gray-200 hover:border-gray-300 text-gray-600"
              }`}
            >
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <TransportAddressPicker
            ...
            detailLabel="Sokak, bina (tam adres)"
            detailPlaceholder="Gönderi nerede?"
        ...
        <div>
          <label className="text-xs text-gray-500 mb-1 block font-semibold">Teslimat *</label>
          <TransportAddressPicker
            trValue={toTr}
            onTrChange={setToTr}
            detail={toDetail}
            ...
            detailLabel="Sokak, bina (tam adres)"
            detailPlaceholder="Nereye teslim edilsin?"
            onGeofill={({ address, lat, lng }) => {
              setToDetail(address);
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Teslim Saati (opsiyonel)</label>
            <input
              type="datetime-local"
      ...
      >
        {loading ? "Gönderiliyor..." : "Kurye Çağır"}
      </button>
    </div>
  );
}
