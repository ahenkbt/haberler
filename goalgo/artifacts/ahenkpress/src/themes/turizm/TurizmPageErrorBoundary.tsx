...
/** Catches turizm page runtime errors — avoids blank white flash after mount. */
export class TurizmPageErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("[turizm]", this.props.label ?? "page", error, info.componentStack);
  }
      ...
      return (
        <div className="mx-auto max-w-xl px-4 py-16 text-center">
          <div className="rounded-2xl border border-amber-200 bg-white p-8 shadow-sm">
            <h2 className="text-lg font-black text-slate-900">Seyahat sayfası yüklenemedi</h2>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <Link
                href={TURIZM.hub}
                className="rounded-full bg-[#0f766e] px-5 py-2.5 text-sm font-black text-white hover:bg-[#0b5f59]"
              >
                Seyahat ana sayfa
              </Link>
              <Link
