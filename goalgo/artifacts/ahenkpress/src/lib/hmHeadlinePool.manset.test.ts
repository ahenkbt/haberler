...
import {
  ...
  buildLatestNewsSideFallbackPool,
  buildMansetTaggedSideFallbackPool,
...
describe("manset pools", () => {
  ...
  it("eski RSS anasayfa manşetine girmez; taze RSS girer", () => {
    const rssFresh = {
      id: "rss:fresh",
      title: "RSS taze",
      source: "rss",
      publishedAt: hoursAgoIso(2),
      imageUrl: "https://example.com/rf.jpg",
    };
    const pool = buildRssAwareHeadlinePool({
      manualItems: [latestA],
      latestItems: [latestA, rss, rssFresh],
      rssEnabled: true,
      rssBootstrapReady: true,
      limit: 8,
      minManual: 1,
    });
    expect(pool.some((x) => x.id === "rss:fresh")).toBe(true);
  ...
  it("yan kart yedek havuzu eski ve tepe manşet haberlerini eler", () => {
    const latestPool = buildLatestNewsSideFallbackPool({
      items: [featuredOld, latestA, latestB, rss],
      sliderItems: [latestA],
      tepeMansetItems: [featuredOld],
    });
    expect(latestPool.map((x) => x.id)).toEqual([3]);

    const mansetTagged = buildMansetTaggedSideFallbackPool({
      items: [featuredOld, latestA],
      sliderItems: [],
      tepeMansetItems: [],
    });
