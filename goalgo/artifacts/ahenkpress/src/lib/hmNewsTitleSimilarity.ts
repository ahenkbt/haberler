import { newsItemRecencyMs, sortNewsByRecency } from "@/lib/hmHeadlinePool";
...
/** Başlıklar aynı haberi işaret ediyorsa true (en az 3 ortak kelime veya güçlü eşleşme). */
...
export type DeferSimilarNewsOptions = {
  minSharedTokens?: number;
  /** Benzer sayılacak yayın zamanı penceresi (ms). */
  proximityMs?: number;
};

/**
 * En yeni haber önde kalır; benzer başlıklı haberler (aynı konu, farklı başlık) geriye itilir.
 */
export function deferSimilarNewsItems<T extends { title?: string | null; publishedAt?: string | null; createdAt?: string | null; date?: string | null }>(
  items: readonly T[],
  opts?: DeferSimilarNewsOptions,
): T[] {
  const minSharedTokens = opts?.minSharedTokens ?? 3;
  const proximityMs = opts?.proximityMs ?? 48 * 60 * 60 * 1000;
  const sorted = sortNewsByRecency([...items]);
  const front: T[] = [];
  const deferred: T[] = [];
  ...
  return [...front, ...sortNewsByRecency(deferred)];
}
