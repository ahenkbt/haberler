...
const featureKeys = [
  { icon: Bot, key: "aiTemplates" },
  { icon: Layers, key: "customAgents" },
  { icon: Brain, key: "contextAware" },
  { icon: Zap, key: "instantUpdates" },
] as const;

const statKeys = [
  { value: "100+", key: "countries" },
  { value: "30+", key: "languages" },
...
export default function LoginPage() {
  ...
  const handleLogin = async (data: LoginFormData) => {
    ...
    try {
      ...
      toast({ title: t("auth.welcomeBack"), description: t("auth.loginPage.loginSuccess") });
  ...
  const getCardTitle = () => {
    switch (activeView) {
      case "login": return t("auth.loginPage.titles.login");
      case "register": return t("auth.loginPage.titles.register");
      case "register-otp": return t("auth.loginPage.titles.registerOtp");
      case "forgot-password": return t("auth.loginPage.titles.forgotPassword");
      case "reset-password": return t("auth.loginPage.titles.resetPassword");
    }
  };

  const getCardDescription = () => {
    switch (activeView) {
      case "login": return t("auth.loginPage.descriptions.login");
      case "register": return t("auth.loginPage.descriptions.register");
      case "register-otp": return t("auth.loginPage.descriptions.registerOtp", { email: registerForm.getValues().email });
      case "forgot-password": return t("auth.loginPage.descriptions.forgotPassword");
      case "reset-password": return t("auth.loginPage.descriptions.resetPassword", { email: forgotPasswordEmail });
    }
  };
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-[#050B1A] via-[#0a1628] to-[#050B1A] relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col justify-between p-12 w-full">
                  ...
                  >
                    ...
                    <div>
                      <p className="text-white">
                        {t("auth.loginPage.accessPrebuilt")}{" "}
                        <span className="font-semibold">{t(`auth.loginPage.features.${feature.key}.title`)}</span>.{" "}
                        <span className="text-gray-400">{t(`auth.loginPage.features.${feature.key}.description`)}</span>
                      </p>
                    </div>
                  </motion.div>
                ))}
                ...
                <div className="grid grid-cols-4 gap-6">
                  {statKeys.map((stat, index) => (
                    <div key={index} className="text-center" data-testid={`stat-${index}`}>
                      ...
                      <div className="text-sm text-gray-400">{t(`auth.loginPage.stats.${stat.key}`)}</div>
                    </div>
                  ))}
                </div>
              </motion.div>
            ...
            <p className="text-sm text-gray-500">
              &copy; {new Date().getFullYear()} {branding.app_name}. {t("auth.loginPage.allRightsReserved")}
            </p>
          </div>
        </div>
                      ...
                      <TabsContent value="login">
                        <form onSubmit={loginForm.handleSubmit(handleLogin)} className="space-y-4">
                          <div className="space-y-2">
                            <Label htmlFor="login-email" className="text-gray-700 dark:text-gray-300">{t("common.email")}</Label>
                            <Input
                              id="login-email"
                              type="email"
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="login-password" className="text-gray-700 dark:text-gray-300">{t("common.password")}</Label>
                          ...
                          >
                            {isLoading ? t("auth.loginPage.signingIn") : t("auth.signIn")}
                          </Button>
                        </form>
                      </TabsContent>
