...
export default function GlobalSettings({ onSwitchTab }: GlobalSettingsProps) {
    ...
    onSuccess: (data) => {
      ...
      toast({ 
        ...
        description: data.enabled 
          ? t("admin.settings.kyc.enabledFor", { provider: providerName })
          : t("admin.settings.kyc.disabledFor", { provider: providerName })
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleSaveGoogleOAuth = async () => {
    ...
    try {
      ...
      toast({ title: t("admin.settings.googleOAuth.saved") });
    } catch (err: any) {
      toast({ title: t("admin.settings.googleOAuth.saveFailed"), description: err?.message, variant: "destructive" });
    } finally {
