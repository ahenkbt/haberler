...
const SUPPORTED_LOCALES = ['en', 'ar', 'de', 'es', 'fr', 'hi', 'it', 'ja', 'pl', 'pt', 'sv', 'tr'] as const;

const viteDefaultLocale = (import.meta.env.VITE_DEFAULT_LOCALE as string | undefined)
  ?.split('-')[0]
  ?.toLowerCase();

const defaultLocale =
  viteDefaultLocale && SUPPORTED_LOCALES.includes(viteDefaultLocale as (typeof SUPPORTED_LOCALES)[number])
    ? viteDefaultLocale
    : 'tr';

/** Built-in locale JSON bundles (shipped with the app). Used to prefer static TR over stale DB seeds. */
export const staticTranslationBundles: Record<string, Record<string, unknown>> = {
  en,
  ar,
  de,
  es,
