...
router.post('/openai-sip/project-id', async (req: Request, res: Response) => {
  try {
    ...
    await db.execute(sql`
      ...
      VALUES (gen_random_uuid(), 'openai_sip_project_id', ${JSON.stringify(projectId)}::jsonb, 'OpenAI Project ID for SIP integration')
      ON CONFLICT (key) DO UPDATE SET value = ${JSON.stringify(projectId)}::jsonb, updated_at = NOW()
    `);
...
router.post('/openai-sip/webhook-secret', async (req: Request, res: Response) => {
  try {
    ...
    await db.execute(sql`
      INSERT INTO global_settings (id, key, value, description)
      VALUES (gen_random_uuid(), 'openai_sip_webhook_secret', ${JSON.stringify(webhookSecret)}::jsonb, 'OpenAI webhook secret for signature verification')
      ON CONFLICT (key) DO UPDATE SET value = ${JSON.stringify(webhookSecret)}::jsonb, updated_at = NOW()
    `);
