...
router.post("/", async (req, res) => {
  try {
    ...
    if (finalSipHost) {
      ...
      if (!hostnameRegex.test(finalSipHost) && !ipv4Regex.test(finalSipHost)) {
        return res.status(400).json({
          success: false,
          message: "Invalid SIP Host. Must be a valid hostname (e.g., sip.provider.com) or IP address."
        });
      }
      if (ipv4Regex.test(finalSipHost)) {
        const parts = finalSipHost.split(".").map(Number);
      ...
      if (providerInfo.hostPattern && !ipv4Regex.test(finalSipHost)) {
        if (!providerInfo.hostPattern.test(finalSipHost)) {
          return res.status(400).json({
            success: false,
