...
- Uses GPT-4o Realtime for voice conversations
...
4. Copy this ID for use in AgentLabs admin settings

### Step 2: Configure Webhook in OpenAI Platform

1. In OpenAI Platform, go to **Settings** > **Project** > **Webhooks**
2. Click **Add Webhook**
3. Enter the Webhook URL from AgentLabs admin panel:
   ```
   https://your-domain.com/api/openai-sip/webhook
   ```
4. Select event: `realtime.call.incoming`
5. Save and copy the **Webhook Secret** for AgentLabs

### Step 3: Configure AgentLabs Admin Settings
...
### Step 5: Import Phone Numbers in AgentLabs
...
3. OpenAI sends a webhook to AgentLabs
4. AgentLabs accepts the call with your agent's configuration
...
- Check webhook secret matches between OpenAI and AgentLabs
...
- Ensure phone number is properly imported in AgentLabs

### Agent Not Responding

- Verify agent is assigned to the phone number
- Check agent has a valid system prompt configured
- Review call logs in AgentLabs for error details
