...
The SIP Engine Plugin enables AgentLabs customers to use their **own SIP trunks** for AI-powered voice calls. This "Bring Your Own Trunk" (BYOT) approach allows businesses to leverage existing telephony infrastructure while connecting it to powerful AI voice engines.
...
| **SIP Phone Number Import** | Import phone numbers from your SIP provider into AgentLabs |
...
**Description:** OpenAI Realtime API for incoming SIP calls. Uses GPT-4o Realtime for voice conversations with native SIP support.
...
5. AgentLabs stores the mapping
...
1. Create a **Campaign** in AgentLabs
...
6. Call completion updates are sent to AgentLabs
...
4. OpenAI sends webhook to AgentLabs: `/api/openai-sip/webhook`
5. AgentLabs looks up phone number and agent
6. AgentLabs accepts call with agent configuration
7. OpenAI handles the conversation

**Webhook Event Flow:**
```
OpenAI SIP → AgentLabs Webhook → Accept/Reject → OpenAI Realtime API
```
...
3. Check webhook secret matches between OpenAI and AgentLabs
...
```bash
grep "\[SIP\]" /var/log/agentlabs.log
grep "\[ElevenLabs SIP\]" /var/log/agentlabs.log
grep "\[OpenAI SIP\]" /var/log/agentlabs.log
```
...
3. Contact support@agentlabs.io
