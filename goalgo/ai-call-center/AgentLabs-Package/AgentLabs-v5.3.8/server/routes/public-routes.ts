      ...
      try {
        ...
        const code =
          error && typeof error === "object" && "code" in error
            ...
            : "";
        const hint =
          /ssl|certificate|TLS/i.test(detail) || code === "28000"
            ? " Railway Postgres için DATABASE_URL linkli mi ve son deploy SSL düzeltmesini içeriyor mu kontrol edin."
            : "";
        checks.push({
          name: "Database Connection",
          status: "error",
          message: detail
            ? `Database connection failed${code ? ` (${code})` : ""}: ${detail}${hint}`
