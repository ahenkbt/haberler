...
@section('content')
    <div class="page-header">
        <h4 class="page-title">{{ __('Edit_Testimonial') }}</h4>
        <ul class="breadcrumbs">
            ...
            <li class="nav-item">
                <a href="#">{{ __('Testimonial_Page') }}</a>
            </li>
            <li class="separator">
                <i class="flaticon-right-arrow"></i>
            </li>
            <li class="nav-item">
                <a href="#">{{ __('Edit_Testimonial') }}</a>
            </li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="card-title d-inline-block">{{ __('Edit_Testimonial') }}
                    </div>
                            ...
                            <form id="ajaxForm" class="" action="{{ route('user.testimonial.update') }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="name"
                                        value="{{ $testimonial->name }}" placeholder="{{ __('Enter_Name') }}">
                                ...
                                @if ($userBs->theme !== 'home_nine')
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="occupation"
                                            value="{{ $testimonial->occupation }}" placeholder="{{ __('Enter_occupation') }}">
                                        <p id="erroccupation" class="mb-0 text-danger em"></p>
                                    </div>
                                @endif
                                <div class="form-group">
                                    <label for="">{{ __('Feedback') }} **</label>
                                    <textarea class="form-control summernote" name="content" rows="5" placeholder="{{ __('Enter_content') }}">{{ replaceBaseUrl($testimonial->content) }}</textarea>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="serial_number"
                                        value="{{ $testimonial->serial_number }}" placeholder="{{ __('Enter_Serial_Number') }}">
                                    <p id="errserial_number" class="mb-0 text-danger em"></p>
                                    <p class="text-warning">
                                        <small>{{ __('Testimonial_Serial_Number_text') }}</small>
                                    </p>
                                </div>
                            </form>
                        </div>
