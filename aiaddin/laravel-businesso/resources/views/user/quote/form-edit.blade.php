                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 text-right">
                        <a class="btn btn-primary"
                            href="{{ route('user.quote.form') . '?language=' . request()->input('language') }}">{{ __('Back') }}</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            <div class="row" id="app">
                ...
                <div class="col-lg-6 offset-lg-3">
                    <form id="ajaxForm" action="{{ route('user.quote.inputUpdate') }}" method="post"
                                ...
                                <div class="row mb-2 counterrow" v-for="n in counter" :id="'counterrow' + n">
                                    <div class="col-md-11">
                                        <input class="form-control optionin" type="text" name="options[]"
                                            :value="names[n - 1]" placeholder="{{ __('Option_label') }}">
                                    </div>
                        ...
                        <div class="text-center form-group">
                            <button id="submitBtn" type="submit" class="btn btn-primary">{{ __('UPDATE_FIELD') }}</button>
                        </div>
                    </form>
                </div>
            </div>
...
@section('vuescripts')
    <script>
        ...
        var app = new Vue({
            ...
            created() {
                $.get("{{ route('user.quote.options', $input->id) }}", (data) => {
