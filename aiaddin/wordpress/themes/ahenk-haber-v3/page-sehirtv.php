...
.sehirtv-baslik span { background: #CC0000; color: #fff; padding: 4px 14px; border-radius: 4px; font-size: 14px; }
</style>
...
<div class="sehirtv-wrap">
    ...
    if ( function_exists('VTV_Shortcode::render') || shortcode_exists('video_tv') ) {
        ...
        echo '<h2 style="color:#CC0000;margin:0 0 10px">Video TV Eklentisi Aktif Değil</h2>';
        echo '<p style="color:#666">Video TV eklentisini etkinleştirerek bu sayfayı kullanmaya başlayabilirsiniz.</p>';
        if ( current_user_can('manage_options') ) {
            echo '<a href="' . admin_url('plugins.php') . '" style="display:inline-block;margin-top:12px;background:#CC0000;color:#fff;padding:10px 24px;border-radius:6px;font-weight:700">Eklentilere Git</a>';
        }
        echo '</div>';
    }
    ?>
