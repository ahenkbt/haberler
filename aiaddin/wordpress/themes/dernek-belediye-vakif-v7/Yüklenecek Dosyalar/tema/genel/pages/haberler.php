...
$page = @intval($_GET['s']);
if(!$page) $page = 1;
$ttsorgu = $db->prepare("SELECT COUNT(*) FROM haber_kategori WHERE durum = ? AND dil = ?");
$ttsorgu->execute(array("1",$_SESSION['k_dil']));
$total = $ttsorgu->fetchColumn();
$limit= $limitayar['limit_sayfahaberler'];
$page_count = ceil($total/$limit);
if($page > $page_count) $page = 1;
$show = $page * $limit - $limit;
$BSorgu = $db->prepare("SELECT * FROM haber_kategori WHERE durum = ? AND dil = ? ORDER BY sira ASC LIMIT $show,$limit");
$BSorgu->execute(array("1",$_SESSION['k_dil']));
$Bislem = $BSorgu->fetchALL(PDO::FETCH_ASSOC);
$menubul 	= $db->query("SELECT * FROM menu WHERE menu_url = '".$htc['haberlerurl']."' OR link = '".$htc['haberlerurl']."' AND dil = '{$_SESSION['k_dil']}'")->fetch(PDO::FETCH_ASSOC);
...
<section class="page-section">
	<div class="bg-white">
		<div class="col-12 p-0 banner">
			<img src="<?php echo tema;?>/uploads/arkaplan/arkaplan19/<?php echo $arkaplan['arkaplan19'];?>" alt="<?=@$dil['txt152'];?>">
			...
			<div class="row">
				...
				<div class="col-lg-<?php echo($moduller['alan23'] == "1" ? '9' : '12');?> col-md-<?php echo($moduller['alan23'] == "1" ? '7' : '12');?> z-index-9">
					<div class="page-content">
						<h2 class="page-title"><?=@$dil['txt152'];?></h2>
						<div class="gallery list">
							<?php if($BSorgu->rowCount() != "0"){?>
							<ul class="row">
								<?php foreach ( $Bislem as $BSonuc ){?>
								<li class="gallery-card col-md-<?php echo $limitayar['limit_haberler'];?> col-lg-<?php echo $limitayar['limit_haberler'];?>">
									<a href="<?php echo $htc['haberkategoriurl']; ?>/<?php echo $BSonuc['seo']; ?><?php echo $html;?>">
										<div class="gallery-cover-kapak"><img src="<?php echo tema;?>/uploads/haber_kategoriler/kapak/<?php echo $BSonuc['kapak']; ?>"></div>
							...
							<div class="pagination">
								<ul>
								<?php if($limitayar['limit_sayfahaberler'] < $total && $limitayar['limit_sayfahaberler'] > 0){
								$showing = 3;
								if($page > 1){ $previous = $page - 1;?>
								<li class="onceki_sayfa"><a href="<?php echo $htc['haberurl'];?>/<?php echo $previous;?><?php echo $html;?>"><i class="fas fa-angle-left"></i></a></li>
								<?php }
								...
								<?php  $next = $page +1;?>
								<li class="sonraki_sayfa"><a href="<?php echo $htc['haberurl'];?>/<?php echo $next; ?><?php echo $html;?>"><i class="fas fa-angle-right"></i></a></li>
								<?php }} ?>	
								</ul>
							</div>
							<?php }else{?>
