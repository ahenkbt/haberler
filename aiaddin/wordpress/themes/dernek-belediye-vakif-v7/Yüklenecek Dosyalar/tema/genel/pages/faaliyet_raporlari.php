...
$page = @intval($_GET['s']);
if(!$page) $page = 1;
$ttsorgu = $db->prepare("SELECT COUNT(*) FROM faaliyet_raporlari WHERE durum = ? AND dil = ?");
$ttsorgu->execute(array("1",$_SESSION['k_dil']));
$total = $ttsorgu->fetchColumn();
$limit= $limitayar['limit_sayfafaaliyet'];
$page_count = ceil($total/$limit);
if($page > $page_count) $page = 1;
$show = $page * $limit - $limit;
$BSorgu = $db->prepare("SELECT * FROM faaliyet_raporlari WHERE durum = ? AND dil = ? ORDER BY sira ASC LIMIT $show,$limit");
...
<section class="page-section">
	<div class="bg-white">
		<div class="col-12 p-0 banner">
			<img src="<?php echo tema;?>/uploads/arkaplan/arkaplan8/<?php echo $arkaplan['arkaplan8'];?>" alt="<?=@$dil['txt149'];?>">
			...
			<div class="row">
				...
				<div class="col-lg-<?php echo($moduller['alan23'] == "1" ? '9' : '12');?> col-md-<?php echo($moduller['alan23'] == "1" ? '7' : '12');?> z-index-9">
					<div class="page-content">
						<h2 class="page-title"><?=@$dil['txt149'];?></h2>
						<div class="documents">
						<?php if($BSorgu->rowCount() != "0"){?>
							<ul class="row mr-0 ml-0">							
								<?php foreach ( $Bislem as $BSonuc ){?>
								<li class="col-lg-<?php echo $limitayar['limit_faaliyet'];?> pl-0 pr-0">
									<a href="<?php echo $htc['faaliyetdetayurl']; ?>/<?php echo $BSonuc['seo']; ?><?php echo $html;?>">
									...
									<div class="documents-sub">
										<div class="documents-sub-content">
											...
											<div class="documents-files documents-icon">
												<ul>
													<li>
														<a class="row no-gutters" target="_blank" href="<?php echo tema;?>/uploads/faaliyet_raporlari/dosya/<?php echo $BSonuc['dosya'];?>" data-documents-src="">
															<div class="documents-icon" data-documents-icon="">
																<i class="far fa-file"></i>
															</div>
															<div class="col">
																<div class="text"><?php echo $BSonuc['adi'];?></div>
																<div class="date"><?php echo tarih2($BSonuc['tarih']);?></div>
															</div>
							...
							<div class="pagination">
								<ul>
								<?php if($limitayar['limit_sayfafaaliyet'] < $total && $limitayar['limit_sayfafaaliyet'] > 0){
								$showing = 3;
								if($page > 1){ $previous = $page - 1;?>
								<li class="onceki_sayfa"><a href="<?php echo $htc['faaliyeturl'];?>/<?php echo $previous;?><?php echo $html;?>"><i class="fas fa-angle-left"></i></a></li>
								<?php }
								...
								<?php }else{?>
								<li><a href="<?php echo $htc['faaliyeturl'];?>/<?php echo $i; ?><?php echo $html;?>"><?php echo $i; ?></a></li>
								<?php } } } if($page != $page_count){?>
								<?php  $next = $page +1;?>
								<li class="sonraki_sayfa"><a href="<?php echo $htc['faaliyeturl'];?>/<?php echo $next; ?><?php echo $html;?>"><i class="fas fa-angle-right"></i></a></li>
								<?php }} ?>	
								</ul>
							</div>
							<?php }else{?>
