...
$limit= $limitayar['limit_sayfafoto'];
$page_count = ceil($total/$limit);
if($page > $page_count) $page = 1;
$show = $page * $limit - $limit;
$BSorgu = $db->prepare("SELECT * FROM foto_galeri WHERE durum = ? AND dil = ? ORDER BY sira ASC LIMIT $show,$limit");
...
<section class="page-section">
	<div class="bg-white">
		<div class="col-12 p-0 banner">
			<img src="<?php echo tema;?>/uploads/arkaplan/arkaplan14/<?php echo $arkaplan['arkaplan14'];?>" alt="<?=@$dil['txt151'];?>">
			...
			<div class="row">
				...
				<div class="col-lg-<?php echo($moduller['alan23'] == "1" ? '9' : '12');?> col-md-<?php echo($moduller['alan23'] == "1" ? '7' : '12');?> z-index-9">
					<div class="page-content">
						<h2 class="page-title"><?=@$dil['txt151'];?></h2>
						<div class="gallery list">
							<?php if($BSorgu->rowCount() != "0"){?>
							<ul class="row">
								<?php foreach ( $Bislem as $BSonuc ){?>
								<li class="gallery-card col-md-<?php echo $limitayar['limit_foto'];?> col-lg-<?php echo $limitayar['limit_foto'];?>">
									<a href="<?php echo $htc['fotodetayurl']; ?>/<?php echo $BSonuc['seo']; ?><?php echo $html;?>">
										<div class="gallery-cover-kapak"><img src="<?php echo tema;?>/uploads/fotogaleri/kapak/<?php echo $BSonuc['kapak']; ?>"></div>
							...
							<div class="pagination">
								<ul>
								<?php if($limitayar['limit_sayfafoto'] < $total && $limitayar['limit_sayfafoto'] > 0){
								$showing = 3;
								if($page > 1){ $previous = $page - 1;?>
								<li class="onceki_sayfa"><a href="<?php echo $htc['fotourl'];?>/<?php echo $previous;?><?php echo $html;?>"><i class="fas fa-angle-left"></i></a></li>
								<?php }
								...
								<?php }else{?>
								<li><a href="<?php echo $htc['fotourl'];?>/<?php echo $i; ?><?php echo $html;?>"><?php echo $i; ?></a></li>
								<?php } } } if($page != $page_count){?>
								<?php  $next = $page +1;?>
								<li class="sonraki_sayfa"><a href="<?php echo $htc['fotourl'];?>/<?php echo $next; ?><?php echo $html;?>"><i class="fas fa-angle-right"></i></a></li>
								<?php }} ?>	
								</ul>
							</div>
							<?php }else{?>
